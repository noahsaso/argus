 ...
 */
export const parseAllowanceData = (
  ...
  try {
    ...
    if (dataStr.includes('636f696e')) {
      // "coin" in hex
      // Likely contains coin data
      allowanceType = 'BasicAllowance'
    } else if (dataStr.includes('706572696f64')) {
      // "period" in hex
      allowanceType = 'PeriodicAllowance'
    } else if (dataStr.includes('616c6c6f776564')) {
      // "allowed" in hex
    ...
    try {
      ...
      if (dataStr.includes('7578696f6e')) {
        // "uxion" in hex
        denom = 'uxion'
        ...
        if (uxionIndex > 0) {
          ...
          if (amountMatch) {
            try {
              ...
              const _hexAmount = amountMatch[1]
      ...
      else if (dataStr.includes('7575736463')) {
        // "uusdc" in hex
        denom = 'uusdc'
 ...
 */
export const parseAllowanceDataAdvanced = (
  allowanceData: string
): ParsedAllowanceData => {
 ...
 */
export const formatAllowanceAmount = (
  amount: string,
  denom: string
): string => {
  if (!amount || amount === '0') {
